package com.example.pss.support.mq.rabbit;
...
@Slf4j
@Service
@ConditionalOnProperty(name = "pss.mq.pipeline", havingValue = MessageQueuePipeline.RABBIT_MQ)
public class RabbitSendMqServiceImpl implements SendMqService {

    @Autowired
    private RabbitTemplate rabbitTemplate;

    @Value("${pss.rabbitmq.topic.name}")
    private String confTopic;

    @Value("${pss.rabbitmq.exchange.name}")
